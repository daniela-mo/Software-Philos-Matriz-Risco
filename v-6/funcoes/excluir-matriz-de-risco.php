...
if ($excluir) {
    ...
    $seleco = mysqli_query($conexao, "select * from identificacao_do_risco order by codigo_matriz_risco DESC");
    $registr = mysqli_fetch_array($seleco);
    $codigoa = $registr['codigo'];


    $excluir6 = mysqli_query($conexao, "delete  from item_qaa_risco WHERE codigo_matriz_risco='$codigoa'");
    $excluir7 = mysqli_query($conexao, "delete  from demais_areas_risco WHERE codigo_matriz_risco='$codigoa'");
    $excluir8 = mysqli_query($conexao, "delete  from area_principal_risco WHERE codigo_matriz_risco='$codigoa'");
} else { ?>
